<?php
/**
 * dashboard2.php - Centro de Comando Geoespacial (v23.2 - Con Popup de Eventos)
 */
global $pdo;
...
<style>
    ...
    /* ===== INICIO: NUEVOS ESTILOS PARA EL POPUP DE EVENTOS EN VIVO ===== */
    .event-popup {
        position: fixed;
        bottom: -300px; /* Inicia fuera de la pantalla */
        right: 20px;
        width: 450px;
    ...
    .popup-details p { margin: 5px 0; color: var(--secondary-text-color); }
    .popup-details p i { color: var(--primary-yellow); margin-right: 8px; width: 16px; text-align: center; }
    .popup-decision { position: absolute; top: 0; right: 0; bottom: 0; width: 10px; }
    .popup-decision.decision-green { background: var(--decision-green); }
    .popup-decision.decision-red { background: var(--decision-red); }
    .popup-info-container .styled-plate-mercosur { transform: scale(0.9); transform-origin: left; }
    /* ===== FIN: NUEVOS ESTILOS PARA EL POPUP DE EVENTOS EN VIVO ===== */
</style>
...
<!-- ===== INICIO: NUEVO POPUP DE EVENTOS EN VIVO ===== -->
<div id="event-popup" class="event-popup">
    <div class="popup-content">
        <div class="popup-image-container">
            <img id="popup-image" src="images/placeholder.jpg" alt="Captura de matrícula">
        </div>
        <div class="popup-info-container">
            <div id="popup-plate" class="styled-plate-mercosur">
                <div class="plate-inner">
                    <div class="plate-header"><span>MERCOSUR</span><span>ARG</span></div>
                    <div class="plate-number">CARGANDO</div>
                </div>
            </div>
            <div class="popup-details">
                <p><i class="fas fa-user"></i> Propietario: <strong id="popup-owner">--</strong></p>
                <p><i class="fas fa-camera"></i> Cámara: <strong id="popup-device">--</strong></p>
                <p><i class="fas fa-clock"></i> Hora: <strong id="popup-time">--</strong></p>
            </div>
        </div>
        <div id="popup-decision" class="popup-decision"></div>
    </div>
</div>
<!-- ===== FIN: NUEVO POPUP DE EVENTOS EN VIVO ===== -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let popupMap = null;
    let notificationTimeout; // <--- Temporizador para el modal de notificación

    // --- Referencias a los KPIs (sin cambios) ---
    const kpiValues = {
    ...
    // --- Lógica del Lazy Load Observer (sin cambios) ---
    ...
    // --- Lógica para renderizar el Timeline (sin cambios) ---
    ...
    async function updateDashboard() {
        try {
            ...
            // ... (Lógica de actualización de KPIs se mantiene) ...
            ...
            // --- Lógica de Notificación con Modal ---
            if (data.latest_entries.length > 0 && lastEntryId !== 0 && data.latest_entries[0].id > lastEntryId) {
                openDetailsPopup(data.latest_entries[0].id, true); // <--- CAMBIO
            }
            ...
            if (data.latest_exits.length > 0 && lastExitId !== 0 && data.latest_exits[0].id > lastExitId) {
                openDetailsPopup(data.latest_exits[0].id, true); // <--- CAMBIO
            }
    ...
    // --- Lógica para abrir el Popup de Detalles (Manual y por Notificación) ---
    async function openDetailsPopup(eventId, isNotification = false) {
        if (isNotification && notificationTimeout) {
    ...
    // Event listener para abrir el popup manualmente (sin cambios)
    ...
    // Función para renderizar el contenido del popup (sin cambios)
    ...
    // Función para inicializar el mapa del popup (sin cambios)
