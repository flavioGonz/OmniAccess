<?php
/**
 * manage_lots.php (Versión 3.1 - Con Toasts para Notificaciones)
 */
...
// --- LÓGICA DE PROCESAMIENTO POST (No cambia) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validate_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'save_lot') {
        ...
        if (!empty($description)) {
            ...
            } catch (PDOException $e) { $_SESSION['error_message'] = "Error de base de datos: " . $e->getMessage(); }
        } else { $_SESSION['error_message'] = "La descripción del lote no puede estar vacía."; }
        ...
        } else { $_SESSION['error_message'] = "Faltan datos para asignar el propietario."; }
...
$sql = "SELECT l.id, l.description, l.lot_uid, l.latitude, l.longitude, COUNT(ola.owner_id) AS owner_count FROM lots l LEFT JOIN owner_lot_associations ola ON l.id = ola.lot_id $where_sql GROUP BY l.id ORDER BY l.description ASC";
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($lots as $lot): ?>
                                <tr data-lot-id="<?= $lot['id'] ?>" data-lot-description="<?= htmlspecialchars($lot['description']) ?>" data-lot-uid="<?= htmlspecialchars($lot['lot_uid'] ?? '') ?>" data-lat="<?= htmlspecialchars($lot['latitude'] ?? '') ?>" data-lon="<?= htmlspecialchars($lot['longitude'] ?? '') ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        showToast('<?= addslashes(htmlspecialchars($toast_message)) ?>', '<?= $toast_type ?>');
    });
</script>
<?php endif; ?>
    ...
    if (document.getElementById('lots-map')) {
        ...
        lotsData.forEach(lot => {
            if (lot.latitude && lot.longitude) {
                ...
                L.marker(latLng, { icon: lotIcon }).addTo(window.map).bindPopup(`<b>Lote:</b> ${lot.description}`);
                bounds.push(latLng);
            }
        });
