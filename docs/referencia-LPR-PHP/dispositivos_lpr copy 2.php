<?php
// dispositivos_lpr.php (Versión Completa y Modificada con campo "purpose")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $purpose = trim($_POST['purpose'] ?? 'undefined'); // CAMBIO: Añadido campo purpose
        $ip = trim($_POST['ip'] ?? '');
            ...
            if (empty($name) || empty($ip) || empty($mac_address) || empty($username)) {
                ...
                if (empty($error_messages)) {
                    if (!empty($id)) {
                        ...
                        $sql = "UPDATE devices SET name = :name, purpose = :purpose, ip = :ip, mac_address = :mac_address, username = :username WHERE id = :id";
                        $params = [':name' => $name, ':purpose' => $purpose, ':ip' => $ip, ':mac_address' => $mac_address, ':username' => $username, ':id' => $id];
                        if (!empty($password)) {
                            $sql = "UPDATE devices SET name = :name, purpose = :purpose, ip = :ip, mac_address = :mac_address, username = :username, password = :password WHERE id = :id";
                        ...
                        if (empty($password)) $error_messages[] = "La contraseña es obligatoria para nuevos dispositivos.";
                            ...
                            try {
                                ...
                                if ($e->getCode() === '23000') $error_messages[] = "Error: El nombre o la MAC Address ya existen.";
...
// CAMBIO: Añadido 'purpose' a la consulta
                ...
                <tbody>
                    <?php foreach ($devices as $device): ?>
                        <tr data-device-id="<?php echo $device['id']; ?>">
                            ...
                            <td>
                                <?php
                                $purpose_text = 'Indefinido';
                                $purpose_class = 'text-gray'; // Asegúrate de tener esta clase en tu CSS o usa un color por defecto
                                if ($device['purpose'] === 'entry') {
                                    $purpose_text = 'Entrada';
                                    $purpose_class = 'decision-green-text';
                                } elseif ($device['purpose'] === 'exit') {
                                    $purpose_text = 'Salida';
                                    $purpose_class = 'decision-red-text';
                                }
                            ...
                            <td>
                                ...
                                <button class="btn btn-secondary edit-btn" 
                                    ...
                                    data-purpose="<?= htmlspecialchars($device['purpose']) ?>"
            ...
            <form id="device-form" action="index.php?page=dispositivos_lpr" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="device-purpose">Propósito del Dispositivo:</label>
                        <select id="device-purpose" name="purpose" class="form-input" required>
                            <option value="entry">Entrada</option>
                            <option value="exit">Salida</option>
                            <option value="undefined">Indefinido / Otro</option>
                        </select>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const devicePurposeInput = document.getElementById('device-purpose'); // CAMBIO: Referencia al nuevo campo
    ...
    function resetDeviceForm() {
        ...
        devicePasswordInput.required = true;
    }
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            devicePurposeInput.value = this.dataset.purpose; // CAMBIO: Poblar el campo purpose
            ...
            devicePasswordInput.required = false;
    ...
    const closeViewWhitelistButtons = viewWhitelistModal.querySelectorAll('.close-button, .close-button-whitelist-modal');
    ...
    async function checkAllDevicesStatus() {
        try {
            ...
            statuses.forEach(deviceStatus => {
                ...
                if (ledElement) {
                    ledElement.classList.remove('pending', 'ok', 'error'); // Limpiar clases anteriores
                    ledElement.classList.add(deviceStatus.status); // Usa 'ok' o 'error'
