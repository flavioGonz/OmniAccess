<?php
// manage_owners_vehicles.php (Versión 2.1 - Título no pegajoso)
...
// --- LÓGICA DE PROCESAMIENTO POST (Añadir/Editar/Eliminar Propietarios y Vehículos) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'save_owner') {
            ...
            if (!empty($name)) {
                if (!empty($owner_id)) { // Editar propietario
                ...
                } else { // Añadir nuevo propietario
                    // ===== INICIO DE LA CORRECCIÓN =====
                    // Generar un UID único para el nuevo propietario
            ...
            if ($owner_id_to_delete) {
                // Se asume que la FK en la tabla `vehicles` tiene `ON DELETE SET NULL`
                $stmt = $pdo->prepare("DELETE FROM owners WHERE id = :id");
                $stmt->execute([':id' => $owner_id_to_delete]);
                $_SESSION['success_message'] = "Propietario eliminado. Sus vehículos han sido desvinculados.";
            }
        ...
        // --- ACCIONES SOBRE VEHÍCULOS ---
            ...
            if ($owner_id && !empty($plate)) {
                ...
                $_SESSION['error_message'] = "Faltan datos para asignar el vehículo (Propietario y Matrícula son obligatorios).";
            }
        } elseif ($action === 'unassign_vehicle') {
            $vehicle_id_to_unassign = $_POST['vehicle_id_to_unassign'] ?? null;
            if ($vehicle_id_to_unassign) {
                // Simplemente quitamos la vinculación
...
// Consulta principal para obtener todos los propietarios y contar sus vehículos
...
$sql = "
    SELECT o.id, o.name, o.phone, COUNT(v.id) AS vehicle_count
    FROM owners o
...
// Si se ha pedido ver los vehículos de un propietario, los cargamos
$vehicles_of_owner = [];
$owner_in_modal = null;
...
<!-- === CAMBIO: Se eliminó la clase "main-header" para que no sea sticky === --> 
...
<!-- Modal para Añadir/Editar Propietario -->
...
<!-- Modal para Gestionar Vehículos de un Propietario -->
