<?php
/**
 * manage_lots.php (Versión 3.3 - Con Popups de Mapa Mejorados)
 */
...
$toast_message = null;
$toast_type = 'success';
if (isset($_SESSION['success_message'])) { $toast_message = $_SESSION['success_message']; unset($_SESSION['success_message']); }
if (isset($_SESSION['error_message'])) { $toast_message = $_SESSION['error_message']; $toast_type = 'error'; unset($_SESSION['error_message']); }
...
$sql = "
    SELECT 
        ...
        l.latitude, 
        l.longitude, 
        COUNT(DISTINCT ola.owner_id) AS owner_count,
        GROUP_CONCAT(DISTINCT o.name ORDER BY o.name SEPARATOR '|') as owner_names,
    ...
    LEFT JOIN owners o ON ola.owner_id = o.id
    LEFT JOIN vehicles v ON o.id = v.owner_id
    $where_sql
    GROUP BY l.id, l.description, l.lot_uid, l.latitude, l.longitude
    ...
    document.querySelector('table tbody').addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('pan-to-map')) {
            ...
            if (lat && lon && window.map) {
                ...
                lotMarkers[row.dataset.lotId].openPopup();
        ...
        lotsData.forEach(lot => {
            if (lot.latitude && lot.longitude) {
                ...
                function createPopupContent(lot) {
                    ...
                    content += `<div class="popup-header">Lote: <strong>${lot.description}</strong></div>`;
                    content += `<div class="popup-uid">UID: ${lot.lot_uid || 'N/A'}</div>`;
                    content += `<hr>`;
                    if (lot.owner_names) {
                        content += `<div class="popup-section-title"><i class="fas fa-user"></i> Titulares</div>`;
                        content += `<ul class="popup-list">`;
                        lot.owner_names.split('|').forEach(name => { content += `<li>${name}</li>`; });
                        content += `</ul>`;
                    }
                    if (lot.vehicle_plates) {
                        content += `<div class="popup-section-title"><i class="fas fa-car"></i> Matrículas</div>`;
                        content += `<div class="popup-plates">`;
                        lot.vehicle_plates.split('|').forEach(plate => { content += `<span class="styled-plate-mercosur-small">${plate}</span>`; });
                        content += `</div>`;
                    }
                    if (!lot.owner_names && !lot.vehicle_plates) {
                        content += `<p class="no-data">Sin propietarios ni vehículos asignados.</p>`;
                    }
                    content += `</div>`;
                    return content;
                }
                ...
                marker.bindPopup(createPopupContent(lot), { className: 'custom-map-popup' });
